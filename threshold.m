...
% Last Modified by GUIDE v2.5 25-Mar-2017 17:19:40
...
try
	...
	if isa(ImageFilename, 'char')
		...
		if length(imageDimensions) == 3
			...
			set(handles.ColorBand, 'string', 'Image RGB');
...
set(handles.panYAxis,'Visible','off');
...
set(handles.btnFlicker,'Visible','off');
...
%     linkaxes([handles.axes1, handles.axes2], 'xy');
	...
	if (colorBand ~= 1) || (numberOfColorBands == 1)
        ...
        message = ['Rule: Variable > ',num2str(g_MinThreshold),' < ',...
...
try
	...
	if axesShowingInMiddleImage == 1
		...
		set(handles.txtBinarizedImage, 'string', 'Masked Image');
	else
...
function radLogCount_DeleteFcn(hObject, eventdata, handles)
% hObject    handle to radLogCount (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
